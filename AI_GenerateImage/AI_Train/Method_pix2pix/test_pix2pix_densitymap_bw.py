...
def main():
    ...
    if args.run_dir:
        ...
        outputs_dir = project_root / "AI_Result" / "Method_pix2pix" / "outputs"
        runs = sorted(list(outputs_dir.glob("run_pix2pix_*")))
        if not runs:
    ...
    test_result_bw.mkdir(parents=True, exist_ok=True)
    test_result_predictions.mkdir(parents=True, exist_ok=True)
    if args.save_colorjet:
        test_result_colorjet.mkdir(parents=True, exist_ok=True)

    device = get_device()
    image_size = cfg.get("image_size", 256)
    target_channels = cfg.get("target_channels", 1)

    model = UNetGenerator(in_ch=3, out_ch=target_channels).to(device)
    checkpoint = torch.load(checkpoint_path, map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()

    test_dataset = Pix2PixDataset(dataset_root, "test", image_size, target_channels)
    ...
    rows = []
    print(f"\n--- Evaluating Method_pix2pix Test Set ({len(test_dataset)} samples) ---")
    ...
    # Save test_evaluation_per_image.csv and test_evaluation_summary.csv
    summary_dict = {}
    if rows:
