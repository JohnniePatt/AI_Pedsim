...
def get_available_methods(ai_train_dir):
    """
    Scans the trajectory AI_Train directory for subfolders starting with 'Method_'.
...
def get_method_config_path(method_path, config_type="train", config_name=None):
    ...
    if not full_path.exists():
        # Fallback for older folders that haven't been split yet (train only)
...
def get_method_runs(method_path):
    """
    Scans for 'outputs' folder within the result method directory
