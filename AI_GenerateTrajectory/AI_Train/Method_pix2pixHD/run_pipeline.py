...
def get_run_directories(script_dir):
    ...
    outputs_dir = project_root / "AI_Result" / "Method_pix2pixHD" / "outputs"
    
    if not outputs_dir.exists():
        return []
        
    run_dirs = [d for d in outputs_dir.iterdir() if d.is_dir() and d.name.startswith("run_HD_")]
...
def guess_test_script(run_path):
    ...
    if config_names:
        ...
        if "bw" in cfg_name:
            return "test_pix2pixHD_densitymap_bw.py"
        elif "03" in cfg_name:
            return "test_pix2pixHD_trajectoryLine_03.py"
        elif "04" in cfg_name:
            return "test_pix2pixHD_trajectoryLine_04.py"
        elif "02" in cfg_name:
            return "test_pix2pixHD_trajectoryLine.py"
        else:
            return "test_pix2pixHD_trajectoryLine.py"
...
def main():
    ...
    train_scripts = sorted([p.name for p in script_dir.glob("train_pix2pixHD_*.py")])
    ...
    if choice == "1":
        ...
        try:
            ...
            if config_choice == "":
                ...
                if "bw" in selected_script:
                    selected_config = "config_train_03_bw.json"
                elif "03" in selected_script:
                    selected_config = "config_train_03.json"
                elif "04" in selected_script:
                    selected_config = "config_train_04.json"
                elif "02" in selected_script:
                    selected_config = "config_train_02.json"
                else:
        ...
        if not guessed_test:
            guessed_test = "test_pix2pixHD_densitymap_bw.py" if "bw" in selected_script else "test_pix2pixHD_trajectoryLine.py"
            
        # Match config_test for guessed script
        guessed_test_config = None
        if "bw" in guessed_test:
            guessed_test_config = "config_test_03_bw.json"
        elif "03" in guessed_test:
            guessed_test_config = "config_test_03.json"
        elif "04" in guessed_test:
            guessed_test_config = "config_test_04.json"
        elif "02" in guessed_test:
            guessed_test_config = "config_test_02.json"
        ...
        test_scripts = sorted([p.name for p in script_dir.glob("test_pix2pixHD_*.py")])
        ...
        # Select test config
        test_configs = sorted([p.name for p in script_dir.glob("config_test_*.json")])
