"""
special_test_pix2pixHD_trajectory.py
------------------------------------
Run a single-case special test for Method_pix2pixHD.
"""
...
from test_pix2pixHD_trajectoryLine import GeneratorNetwork
...
def load_generator(run_path: pathlib.Path, device: torch.device) -> tuple[GeneratorNetwork, pathlib.Path]:
    ...
    if ckpt is None:
        raise FileNotFoundError(f"No pix2pixHD checkpoint found in {run_path}")
...
def main(run_path: str, input_case_dir: str, output_dir: str, plan_key: str | None = None):
    ...
    print(f"[SpecialTest] Method_pix2pixHD case={case_id}")
