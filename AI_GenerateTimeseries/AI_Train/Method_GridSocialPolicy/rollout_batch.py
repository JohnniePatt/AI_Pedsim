...
def main() -> None:
    ...
    if "sqlite_stem" in split_df.columns:
        split_df = split_df[split_df["sqlite_stem"].str.contains("_full")].reset_index(drop=True)
    if "plan_name" in split_df.columns:
        split_df = split_df.drop_duplicates(subset=["plan_name"]).reset_index(drop=True)
    if split_df.empty:
