...
def assert_checkpoint_compatible(checkpoint_data: dict, dataset_path: str, allow_dataset_mismatch: bool) -> bool:
...
def main(
    ...
    compatibility_ok = assert_checkpoint_compatible(
        checkpoint_data_cfg, dataset_path, allow_dataset_mismatch
    )
    ...
    eval_layout = create_evaluation_layout(
        ...
        invalid_reason=(
            "checkpoint/dataset mismatch; framing output only"
            if not compatibility_ok else None
        ),
    )
    out_dir = eval_layout.root
    traj_dir = eval_layout.previews
    # Dataset
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Evaluating"):
            ...
            for i in range(len(lengths)):
                ...
                agent_id = int(batch["agent_ids"][i]) if "agent_ids" in batch else i
                if c_id not in case_predictions:
                    case_predictions[c_id] = {"case_dir": c_dir, "agents": []}
                case_predictions[c_id]["agents"].append((ox_w, pred_w, gt_w, agent_id))
    ...
    for c_id, c_data in tqdm(case_predictions.items(), desc="Saving Plots"):
        case_preview_dir = traj_dir / c_id
        case_preview_dir.mkdir(parents=True, exist_ok=True)
        out_img = case_preview_dir / "raw_rollout.png"
        ...
        prediction_rows = []
        for _obs_w, pred_w, _gt_w, agent_id in c_data["agents"]:
            for step_index, (pos_x, pos_y) in enumerate(pred_w):
                prediction_rows.append(
                    {
                        "case_id": c_id,
                        "split": split,
                        "frame": (cfg.get("obs_len", 5) + step_index) * cfg.get("frame_stride", 8),
                        "agent_id": agent_id,
                        "pos_x": float(pos_x),
                        "pos_y": float(pos_y),
                        "is_active": True,
        ...
        write_case_prediction(eval_layout, c_id, pd.DataFrame(prediction_rows), variant="raw")
    ...
    summary_row = {
        ...
        "out_of_bounds_rate": wall_violation_rate,
        "wall_crossing_rate": np.nan,
        "collision_exposure_rate": np.nan,
        "invalid_step_rate": wall_violation_rate,
        "goal_reach_rate": goal_success_rate,
        "exit_flow_error": np.nan,
        "density_map_error": np.nan,
        "constraint_intervention_rate": 0.0,
        "latency_ms_per_agent_step": avg_latency_ms,
        "real_time_factor": np.nan,
    }
    pd.DataFrame([summary_row]).to_csv(eval_layout.metrics / "summary_metrics.csv", index=False)

    with open(eval_layout.reports / "evaluation_summary.md", "w") as f:
        f.write(
    ...
    floorplan_ids = {
        plan_by_case[c_id] for c_id in case_predictions if c_id in plan_by_case
    }
    additional_failures = []
    if case_id:
        additional_failures.append("single-case evaluation is framing only")
    if float(cfg.get("subset_percent", 100.0)) != 100.0:
        additional_failures.append("dataset subset is not a final evaluation")
    research_valid = finalize_evaluation(
        eval_layout,
        case_ids=case_predictions.keys(),
        floorplan_ids=floorplan_ids,
        compatibility_ok=compatibility_ok,
        canonical_test_required=(dataset_id == "housegan_canonical_imagebase_split_v1"),
        additional_failures=additional_failures,
    )
    print(f"[Test] Research valid: {research_valid}")
