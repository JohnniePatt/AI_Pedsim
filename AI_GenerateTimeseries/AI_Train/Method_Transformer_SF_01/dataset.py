...
class PedestrianDataset(Dataset):
    ...
    def __init__(
        ...
        if case_id:
            wanted = case_id.removeprefix("case_")
            case_dirs = [
                path for path in case_dirs
                if os.path.basename(path).removeprefix("case_") == wanted
            ]
    ...
    def _load_case(self, case_dir: str):
        ...
        for agent_id, grp in traj_df.groupby(id_col):
            ...
            if not np.isfinite(norm).all():
                raise ValueError(f"agent {agent_id} has non-finite normalised coordinates")
            # Do not silently clip trajectories: clipping hides geometry/data errors
            # and changes the supervised target.  A small tolerance accommodates
            # floating-point points lying exactly on a polygon boundary.
            if (norm < -1e-4).any() or (norm > 1.0001).any():
                lo = norm.min(axis=0)
                hi = norm.max(axis=0)
                raise ValueError(
                    f"agent {agent_id} leaves the geometry normalisation frame: "
...
def collate_fn(batch: list[dict]) -> dict:
    ...
    return {
        ...
        "agent_ids":      [b["agent_id"] for b in batch],
        "metas":          [b["meta"] for b in batch],
        "case_dirs":      [b["case_dir"] for b in batch],
    }
