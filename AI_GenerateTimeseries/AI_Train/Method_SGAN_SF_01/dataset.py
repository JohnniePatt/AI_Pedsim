...
class TrajectoryDataset(IterableDataset):
    ...
    def __init__(self, data_dir, config, split="train", shuffle=True):
        ...
        # HouseGAN uses split/case_*/trajectory.parquet, while older datasets
        # stored parquet files directly under the split directory. Support both
        # layouts so the canonical case-based dataset is not silently empty.
        flat_files = list(self.data_dir.glob("*.parquet"))
        case_files = list(self.data_dir.glob("case_*/*.parquet"))
        self.all_files = sorted({*flat_files, *case_files})
