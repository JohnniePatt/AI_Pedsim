...
# 📁 Section: Workspace
st.sidebar.markdown('<p class="sidebar-header">AI Module Domain</p>', unsafe_allow_html=True)
TRAJECTORY_ROOT = pathlib.Path(__file__).resolve().parents[1]
PROJECT_ROOT = TRAJECTORY_ROOT.parent

ai_module_domain = st.sidebar.selectbox(
    "AI Module Domain",
    options=["⏱️ AI_GenerateTimeseries", "🖼️ AI_GenerateImage"],
    index=0,
    label_visibility="collapsed",
)

if "Timeseries" in ai_module_domain:
    MODULE_DIR = PROJECT_ROOT / "AI_GenerateTimeseries"
else:
    MODULE_DIR = PROJECT_ROOT / "AI_GenerateImage"

AI_TRAIN_DIR = MODULE_DIR / "AI_Train"
AI_RESULT_DIR = MODULE_DIR / "AI_Result"
...
if st.sidebar.button(
    "🧪 Manual test model (nm)",
    key="btn_manual_test_nm",
    use_container_width=True,
    help="Manual test model normal stage & Terminal Execution Blueprint"
):
    st.session_state.current_nav = "🧪 Manual test model (nm)"
    st.rerun()
...
if st.session_state.current_nav == "🧹 Data Formatter":
    ...
    <style>
        ...
        div[data-testid="stSidebar"] button[kind="secondary"]:has(div:contains("Manual test model")) {
            background-color: rgba(128, 128, 128, 0.2) !important;
            font-weight: 600 !important;
        }
    </style>
...
if navigation == "🚀 Training model":
    if selected_method == "Method_GPT_Knowledge":
        ...
        manual_feed_cmd = [
            "python",
            "AI_GenerateImage/AI_Train/Method_GPT_Knowledge/build_knowledge.py",
            "--config",
            "AI_GenerateImage/AI_Train/Method_GPT_Knowledge/config_build.json",
        ]
...
# --- PAGE: Manual Test Model (nm) ---
elif navigation == "🧪 Manual test model (nm)":
    st.title("Manual test model normal stage")
    st.caption("Standardized evaluation scripts and expected directory architecture blueprint")

    # Header 1: Standard Directory Structure Blueprint
    st.header("1. Standard Directory Structure Blueprint")
    st.info(
        "**Mandatory Standardized Folder Architecture:** Every training and evaluation run across all models "
        "MUST follow this clean 6-part structure. No floating PNG images at the root directory."
    )
    st.code(
        """run_<ID>/ (หรือ run_<ID>_evaluate/)
├── config_train.json       # หรือ config_test.json (Snapshot คอนฟิกที่ใช้ออกแบบ)
├── progress.json           # สรุปภาพรวมสถานะการทำงานล่าสุด
├── logs/
│   ├── training_history.csv # ⭐ บันทึกทุก Metric ในทุก epoch (epoch, train_loss, val_loss, ade_m, fde_m, lr)
│   └── run.log             # บันทึก Log การทำงาน
├── weights/
│   └── best_model.pth      # ⭐ เก็บเฉพาะ BEST Checkpoint ตัวเดียวเท่านั้น (ยกเลิกการเก็บทุก 10 epoch!)
├── samples/                # รูปตัวอย่าง sample ระหว่าง train
└── test_results/
    ├── test_evaluation_summary.csv
    ├── test_case_metrics.csv
    └── predictions/        # ⭐ เก็บรูปผลลัพธ์ PNG ทั้งหมดของการ Test (ไม่ลอยอยู่นอกโฟลเดอร์)""",
        language="text"
    )

    st.divider()

    # Header 1: Time Series Output
    st.header("2. Time Series Output")
    st.markdown("Terminal execution scripts for authentic evaluation on **`Topo_HouseGAN`** dataset:")

    # Header 2: Model 1 - Transformer
    st.subheader("1. Method_Transformer (GoalConditionedGPT2)")
    st.markdown("Run full evaluation for Transformer model on `Topo_HouseGAN` test set into `run_33_evaluate`:")
    st.code(
        """cd /home/johnnie/programming/AI_Pedsim/AI_Pedsim/AI_GenerateTimeseries/AI_Train/Method_Transformer

/home/johnnie/programming/AI_Pedsim/AI_Pedsim-env/bin/python3 test_transformer.py \\
  ...
  --run_path ../../AI_Result/Method_Transformer/outputs/run_33_evaluate""",
    ...
    # Header 2: Model 2 - GNN-CVAE
    st.subheader("2. Method_GNN_CVAE (Graph Neural Network + CVAE)")
    st.markdown("Run full evaluation for GNN-CVAE model on `Topo_HouseGAN` test set into `run_6_evaluate`:")
    st.code(
        """cd /home/johnnie/programming/AI_Pedsim/AI_Pedsim/AI_GenerateTimeseries/AI_Train/Method_GNN_CVAE

/home/johnnie/programming/AI_Pedsim/AI_Pedsim-env/bin/python3 test_gnn_cvae.py \\
  --config config_test.json \\
  --model_path ../../AI_Result/Method_GNN_CVAE/outputs/run_6_evaluate/weights/best_model.pth \\
  --run_path ../../AI_Result/Method_GNN_CVAE/outputs/run_6_evaluate""",
        language="bash"
    )

    # Header 2: Model 3 - Social GAN
    st.subheader("3. Method_SGAN (Social GAN Multimodal)")
    st.markdown("Run full evaluation for Social GAN model on `Topo_HouseGAN` test set into `run_6_evaluate`:")
    st.code(
        """cd /home/johnnie/programming/AI_Pedsim/AI_Pedsim/AI_GenerateTimeseries/AI_Train/Method_SGAN

/home/johnnie/programming/AI_Pedsim/AI_Pedsim-env/bin/python3 test_sgan.py \\
  --config config_test.json \\
  --model_path ../../AI_Result/Method_SGAN/outputs/run_6_evaluate/weights/best_model.pth \\
  --run_path ../../AI_Result/Method_SGAN/outputs/run_6_evaluate""",
        language="bash"
    )

    # Header 2: Model 4 - LSTM Baseline
    st.subheader("4. Method_LSTM_01 (Recurrent Network Baseline)")
    st.markdown("Run full evaluation for LSTM Trajectory model on `Topo_HouseGAN` test set:")
    st.code(
        """cd /home/johnnie/programming/AI_Pedsim/AI_Pedsim/AI_GenerateTimeseries/AI_Train/Method_LSTM_01

/home/johnnie/programming/AI_Pedsim/AI_Pedsim-env/bin/python3 test_lstm_trajectory.py \\
  --config config_test.json \\
  --model_path ../../AI_Result/Method_LSTM_01/outputs/run_LSTM_20260327_184506_evaluate/weights/best_model.pth \\
  --run_path ../../AI_Result/Method_LSTM_01/outputs/run_LSTM_20260327_184506_evaluate""",
        language="bash"
    )

    # Header 2: Model 5 - Method_GPT_Knowledge
    st.subheader("5. Method_GPT_Knowledge (GPT + RAG Knowledge Retrieval)")
    st.markdown("Run 3-Panel official test evaluation for GPT_Knowledge on `Topo_HouseGAN` test cases:")
    st.code(
        """cd /home/johnnie/programming/AI_Pedsim/AI_Pedsim/AI_GenerateTimeseries/AI_Train/Method_GPT_Knowledge

/home/johnnie/programming/AI_Pedsim/AI_Pedsim-env/bin/python3 special_test_gpt_knowledge.py \\
  --config config_test.json \\
  --output_dir ../../AI_Result/Method_GPT_Knowledge/outputs/run_gpt_knowledge_evaluate""",
        language="bash"
    )

    # Header 2: Model 6 - GridSocialPolicy
    st.subheader("6. Method_GridSocialPolicy (Discrete Spatial Grid Policy)")
    st.markdown("Run batch rollout evaluation for GridSocialPolicy model on `Topo_HouseGAN` test set:")
    st.code(
        """cd /home/johnnie/programming/AI_Pedsim/AI_Pedsim/AI_GenerateTrajectoryGrid/AI_Train/Method_GridSocialPolicy

/home/johnnie/programming/AI_Pedsim/AI_Pedsim-env/bin/python3 rollout_batch.py \\
  --config config_train.json \\
  --checkpoint ../../AI_Result/Method_GridSocialPolicy/run_20260507_004920_evaluate/weights/best_model.pth \\
  --output-root ../../AI_Result/Method_GridSocialPolicy/run_20260507_004920_evaluate""",
        language="bash"
    )
