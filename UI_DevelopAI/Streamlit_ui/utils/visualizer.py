            ...
            if snapshot_path.exists():
                try:
                    ...
                    if dataset_root_raw:
                        dataset_root = pathlib.Path(dataset_root_raw)
                        if not dataset_root.is_absolute():
                            # TRAJECTORY_ROOT is parents[2] (AI_GenerateImage)
                            trajectory_root = pathlib.Path(__file__).resolve().parents[2]
                            dataset_root = (trajectory_root / dataset_root).resolve()
...
def show_test_evaluation(run_dir):
    ...
    # 3. Final display for Trajectories & Rollout Previews (Recursive Scanner)
    all_eval_pngs = sorted(list(run_path.glob("**/*.png")), key=lambda x: x.name)
    # Exclude training loss curve graphs
    eval_images = [p for p in all_eval_pngs if "training_history" not in p.name and "loss" not in p.name]

    if eval_images:
        show_grid_representation_and_gt_details(run_path)
        
        st.subheader("🏁 Trajectory & Grid Policy Rollout Comparisons (AI Prediction vs Ground Truth)")
        cols_per_row = 3
        for r_idx in range((len(eval_images) + cols_per_row - 1) // cols_per_row):
            st_cols = st.columns(cols_per_row)
            for c_idx in range(cols_per_row):
                img_idx = r_idx * cols_per_row + c_idx
                if img_idx < len(eval_images):
                    img_path = eval_images[img_idx]
                    st_cols[c_idx].image(str(img_path), caption=f"{img_path.parent.name}/{img_path.name}", use_container_width=True)
        st.divider()
    else:
        st.info(f"No evaluated preview PNGs found under {run_path.name}. Run test evaluation script to generate samples.")


def show_grid_representation_and_gt_details(run_path: pathlib.Path):
    """Displays 3-channel spatial grid representation maps and ground truth simulation data."""
    import matplotlib.pyplot as plt

    st.markdown("### 🧩 Grid Representation & Ground Truth Input Tensors")
    st.info(
        "Input spatial grid tensors (`walkable`, `exit_room`, `spawn_agent`) converted from Ground Truth JuPedSim simulation trajectories. "
        "Each 3-channel crop gives the AI direct spatial perception of walls, exit doors, and neighbor crowd density."
    )
    
    project_root = pathlib.Path(__file__).resolve().parents[3]
    dataset_root = project_root / "Dataset/Data_TrajectoryGrid/Topo_HouseGAN"
    
    sample_case_dir = None
    if dataset_root.exists():
        manifest_p = dataset_root / "manifest_trajectory_grid.csv"
        if manifest_p.exists():
            try:
                m_df = pd.read_csv(manifest_p)
                m_df = m_df[m_df["sqlite_stem"].str.contains("_full")].reset_index(drop=True)
                if not m_df.empty:
                    sample_case_dir = dataset_root / str(m_df.iloc[0]["input_dir"])
            except Exception:
                pass

    target_case_dir = None
    if sample_case_dir:
        # Match input_dir in dataset_root (A -> B)
        rel_p = str(sample_case_dir).replace("/A/", "/B/")
        target_case_dir = pathlib.Path(rel_p)

    if sample_case_dir and sample_case_dir.exists():
        col_g1, col_g2, col_g3, col_g4 = st.columns(4)
        
        # Channel 1: Walkable Area Grid
        with col_g1:
            st.markdown("**1. Channel 1: Walkable Mask**")
            st.caption("Binary grid (1 = Walkable area, 0 = Wall polygon)")
            grid_json = sample_case_dir / "walkablearea_grid.json"
            if grid_json.exists():
                try:
                    payload = json.loads(grid_json.read_text())
                    walkable_arr = np.array(payload["grid"], dtype=np.uint8)
                    fig, ax = plt.subplots(figsize=(4, 4), facecolor="#101820")
                    ax.imshow(walkable_arr, cmap="gray", origin="upper")
                    ax.axis("off")
                    st.pyplot(fig)
                    plt.close(fig)
                except Exception:
                    st.caption("Walkable Grid Map")

        # Channel 2: Exit Goal Map
        with col_g2:
            st.markdown("**2. Channel 2: Exit Goal Map**")
            st.caption("Target door polygon & centroid vector field")
            exit_json = sample_case_dir / "exit_room.json"
            if exit_json.exists():
                try:
                    exit_payload = json.loads(exit_json.read_text())
                    fig, ax = plt.subplots(figsize=(4, 4), facecolor="#101820")
                    poly = exit_payload.get("exit_node", {}).get("polygon", [])
                    if poly:
                        xs = [p[0] for p in poly]
                        ys = [p[1] for p in poly]
                        ax.fill(xs, ys, color="#f59e0b", alpha=0.6)
                        ax.plot(xs, ys, color="#f97316", linewidth=2)
                    ax.axis("off")
                    st.pyplot(fig)
                    plt.close(fig)
                except Exception:
                    st.caption("Exit Room Vector Map")

        # Channel 3: Spawn Points & Occupancy
        with col_g3:
            st.markdown("**3. Channel 3: Agent Spawns**")
            st.caption("Initial N-agent positions & neighbor density")
            spawn_p = sample_case_dir / "spawn_agent.parquet"
            if spawn_p.exists():
                try:
                    sp_df = pd.read_parquet(spawn_p)
                    fig, ax = plt.subplots(figsize=(4, 4), facecolor="#101820")
                    ax.scatter(sp_df["grid_x"], sp_df["grid_row"], c="#22c55e", s=30)
                    ax.axis("off")
                    st.pyplot(fig)
                    plt.close(fig)
                except Exception:
                    st.caption("Agent Spawn Occupancy Map")

        # Column 4: Ground Truth Trajectory Converted to Grid
        with col_g4:
            st.markdown("**4. Ground Truth Trajectory on Grid**")
            st.caption("JuPedSim GT Trajectory (`trajectory.parquet`)")
            gt_parquet = (target_case_dir / "trajectory.parquet") if target_case_dir else None
            if gt_parquet and gt_parquet.exists() and (sample_case_dir / "walkablearea_grid.json").exists():
                try:
                    grid_payload = json.loads((sample_case_dir / "walkablearea_grid.json").read_text())
                    walkable_arr = np.array(grid_payload["grid"], dtype=np.uint8)
                    gt_df = pd.read_parquet(gt_parquet)
                    
                    fig, ax = plt.subplots(figsize=(4, 4), facecolor="#101820")
                    from matplotlib.colors import ListedColormap
                    cmap = ListedColormap(["#101820", "#f3f6f8"])
                    ax.imshow(walkable_arr, cmap=cmap, origin="upper")
                    
                    for _, agent_df in gt_df.groupby("agent_id"):
                        ordered = agent_df.sort_values("frame")
                        ax.plot(ordered["grid_x"], ordered["grid_row"], linewidth=1.2, alpha=0.85)
                    
                    ax.axis("off")
                    st.pyplot(fig)
                    plt.close(fig)
                except Exception:
                    st.caption("GT Trajectory Plot")
            else:
                st.caption("GT Trajectory File Missing")
        st.divider()
