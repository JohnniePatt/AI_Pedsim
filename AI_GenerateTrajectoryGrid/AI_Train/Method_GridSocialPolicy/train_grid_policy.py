...
def make_run_dir(script_dir: pathlib.Path, output_root: pathlib.Path | None) -> pathlib.Path:
    if output_root is None:
        output_root = script_dir.parents[1] / "AI_Result" / "Method_GridSocialPolicy"
...
def main() -> None:
    ...
    run_dir = make_run_dir(script_dir, pathlib.Path(cfg["output_root"]).resolve() if cfg.get("output_root") else None)
    write_json(run_dir / "config_train.json", cfg)
    ...
    for epoch in range(1, epochs + 1):
        ...
        save_checkpoint(run_dir / "checkpoints" / "last.pt", model, optimizer, epoch, cfg, action_space_payload, best_val_loss)
        if val_metrics["loss"] < best_val_loss:
            best_val_loss = val_metrics["loss"]
            save_checkpoint(run_dir / "checkpoints" / "best.pt", model, optimizer, epoch, cfg, action_space_payload, best_val_loss)
    ...
    print(f"[DONE] checkpoint={run_dir / 'checkpoints' / 'best.pt'}")
