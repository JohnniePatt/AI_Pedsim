...
def make_agent_features(agent: dict, frame_agents: list[dict], exit_payload: dict, grid_meta: dict, crop_size: int) -> np.ndarray:
    ...
    with torch.no_grad():
        for frame in range(max_steps + 1):
            ...
            for agent in agents:
                ...
                feature_rows.append(make_agent_features(agent, agents, exit_payload, grid_payload["meta"], crop_size))
